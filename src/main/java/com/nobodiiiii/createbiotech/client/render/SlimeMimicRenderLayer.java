...
public class SlimeMimicRenderLayer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {
	...
	private static final ResourceLocation SLIME_TEXTURE = ResourceLocation.withDefaultNamespace("textures/entity/slime/slime.png");
	...
	private static volatile Field entityRenderersField;
	...
	public static void registerOnAll(EntityRenderDispatcher dispatcher) {
		for (EntityRenderer<? extends Player> renderer : dispatcher.getSkinMap().values())
			registerOn(renderer);
		for (EntityRenderer<?> renderer : entityRenderers(dispatcher).values())
			registerOn(renderer);
	}
	...
	private static Map<?, EntityRenderer<?>> entityRenderers(EntityRenderDispatcher dispatcher) {
		try {
			Field field = entityRenderersField;
			if (field == null) {
				field = EntityRenderDispatcher.class.getDeclaredField("renderers");
				field.setAccessible(true);
				entityRenderersField = field;
			}
			return (Map<?, EntityRenderer<?>>) field.get(dispatcher);
		} catch (ReflectiveOperationException | RuntimeException e) {
			LOGGER.error("Unable to access the entity renderer registry for slime mimic layer registration.", e);
			return Map.of();
		}
	}
	...
	private static void renderFlatCubeFilter(ModelPart.Cube cube, PoseStack poseStack, RenderContext context,
		...
		runWithoutPartInterception(() -> {
			cube.compile(poseStack.last(), baseConsumer, packedLight, overlay, color(1.0f, 1.0f, 1.0f, 1.0f));
		...
		try {
			for (Object polygon : cubePolygons(access, cube)) {
				...
				for (Object vertex : polygonVertices(access, polygon)) {
					...
					consumer.addVertex(transformedPos.x() + normalX * normalOffset,
