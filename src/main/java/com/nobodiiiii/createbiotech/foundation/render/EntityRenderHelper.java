...
public final class EntityRenderHelper {
	...
	public static <T extends Entity> void render(RenderSettings<T> settings, PoseStack poseStack,
		...
		try {
			...
			runWithFancyGraphics(() -> renderWithAssignedRenderer(dispatcher, settings, poseStack, buffer));
	...
	public static void runWithFancyGraphics(Runnable renderCall) {
		...
		GraphicsStatus previousMode = graphicsMode.get();
		graphicsMode.set(GraphicsStatus.FANCY);
		try {
			renderCall.run();
		} finally {
			graphicsMode.set(previousMode);
		}
	}
